...
## Title
Claude Prompt — API Contract for TactiTok

## Purpose
You are working on the **TactiTok** project.

Your job in this interaction is to create or update the **API Contract** document for the repo.

This is the **sixth core document** after the Product Brief, System Boundaries, MVP Spec, System Architecture, and Data Model.  
...
8. If one or more do not exist yet, explicitly say so and proceed using the North Star plus whatever is available.
9. Update/create only what is necessary for this task.
...
- Offline / Sync / Download Behavior
- Security / Access / Error Handling
- Success & Acceptance

Important rules:
- Ask only questions needed to define the **API Contract**.
- Do not ask broad product questions already answered by the North Star.
- Do not re-open Product Brief, System Boundaries, MVP Spec, System Architecture, or Data Model questions unless they directly affect the API surface.
...
- Prioritize questions that help decide:
  - the minimum API surface needed for MVP
  - the main resource shapes
  - what the edge client needs versus what the admin portal needs
  - how download/catalog refresh behavior is exposed
  - how errors and versioning should be handled simply
- If a sensible default is obvious, state it as a proposed assumption instead of blocking.

### Phase 2 — Draft / Update
After I answer, create or update the API Contract document.

You must:
1. Define the minimum MVP API surface needed to support the agreed scope.
2. Identify the main resources, operations, and request/response shapes.
3. Separate:
   - edge client APIs
   - admin/content-management APIs
   - content access/download behavior
4. Define conventions for:
   - identifiers
   - pagination/filtering if needed
   - error responses
   - versioning
   - publish state / metadata updates where relevant
5. Keep the API small enough for **3 student developers / 8–10 weeks**, but continuation-ready.
...
- consistent with the Product Brief, System Boundaries, MVP Spec, System Architecture, and Data Model
- explicit enough to guide backend/frontend implementation
...
If it fails, propose a smaller **v0.1 / v0.05** API surface.
...
- Prefer the **simplest viable API contract** that still supports later extension.
...
- What API endpoints or operations are needed for MVP
- Which APIs are consumed by the edge client versus the admin portal
- What the core resource shapes look like
- How catalog retrieval, feed retrieval, content metadata access, and admin publishing work
- How content download/access is represented
- What error conventions and versioning rules apply
- What API concerns are intentionally deferred
...
## API Contract document requirements
The API Contract document must be:
- bullet-first
...
5. **Consumers and API areas**
6. **Resource overview**
7. **Core operations**
8. **Request / response conventions**
9. **Filtering / search / pagination conventions**
10. **Content access / download conventions**
11. **Error model**
12. **Versioning approach**
13. **Security / access assumptions**
...
- Separate **resource contract** from **implementation details**.
- Be explicit about the API role of:
  - content catalog retrieval
  - feed/reels retrieval
  - library/category browsing
  - search
  - content item detail retrieval
  - admin upload/create/update/publish flows
  - category management
  - device profile / interest preference persistence only if it belongs in server APIs
- If some state is local-only in MVP, say so explicitly instead of inventing unnecessary endpoints.
- Include a **high-level endpoint/operation list in text form**, for example:
  - `GET /content`
  - `GET /feed`
  - `GET /categories`
  - `POST /admin/content`
...
- For each core operation, identify:
  - purpose
  - caller
  - main inputs
  - main outputs
  - important error cases
- Do not write full OpenAPI specs unless necessary for explanation.
- Do not over-design auth, RBAC, analytics ingestion, moderation workflows, or enterprise integrations that are out of scope for MVP.

---

## Guidance for API framing
Unless the user answers otherwise, prefer a simple MVP API shape such as:

### Candidate edge-facing APIs
- **Catalog / content**
  - list content items
  - get content item detail
  - filter by category / interest / type
  - search by text
- **Feed**
  - get reels/feed items for current device interests
- **Categories**
  - list category tree
- **Content access**
  - resolve PDF/video asset URLs or content references
- **Metadata refresh**
  - fetch latest published catalog metadata

### Candidate admin APIs
- **Content management**
  - create content item
  - update metadata
  - attach/upload asset reference
  - publish/unpublish content
- **Category management**
  - create/update/reorder archive only if needed, otherwise keep minimal
- **Interest/tag management**
  - list and maintain controlled vocabulary only if needed in MVP

### Candidate conventions
- REST-style JSON APIs
- simple page/limit pagination only where needed
- simple query-parameter filtering
- consistent error envelope
- explicit published/draft distinction only if required by MVP

### Candidate deferred areas
- user auth / session APIs beyond minimal portal protection assumptions
...
- this document should turn the agreed scope, architecture, and data model into a practical MVP API contract without prematurely over-designing future integrations or enterprise concerns
